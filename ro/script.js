...
const inspectNoticeCopy = {
  kicker: "Atelier Layero",
  title: "Ups, ușa atelierului s-a deschis.",
  text:
    "Poți arunca o privire, dar imaginile și textele Layero sunt cel mai bine la locul lor. Înapoi în galerie, acolo lumina e mai frumoasă."
};
...
function lockPageForInspect() {
  ...
  document.title = "Atelier Layero blocat";
  ...
  const renderLockScreen = () => {
    ...
    screen.setAttribute("aria-label", "Ecran de blocare inspectare Layero");
    screen.innerHTML =
...
function watchDevToolsState() {
  ...
  if (isLikelyOpen && !devToolsWasLikelyOpen) {
    lockPageForInspect();
    console.info(
      "Atelier Layero: în modul inspectare, conținutul rămâne blocat."
    );
  }
...
newsletterForm?.addEventListener("submit", (event) => {
  ...
  if (!newsletterForm.checkValidity()) {
    if (newsletterStatus) {
      newsletterStatus.textContent =
        "Te rugăm să introduci adresa de email și să accepți condițiile newsletterului.";
    }
...
newsletterCopyButton?.addEventListener("click", async () => {
  ...
  if (newsletterCopyStatus) {
    newsletterCopyStatus.textContent = copied
      ? "Codul cuponului a fost copiat."
      : "Codul tău de cupon: " + newsletterCouponCode;
  }
});
...
if (knowledgeFilterButtons.length && knowledgeCards.length) {
  ...
  const updateKnowledgeHash = (filter) => {
    const nextHash = filter === "all" ? "toate-articolele" : filter;
  ...
  const setKnowledgeFilter = (filter, { updateHash = true } = {}) => {
    ...
    if (knowledgeFilterStatus) {
      knowledgeFilterStatus.textContent = `${visibleCount} intrări vizibile.`;
    }
...
function setProductGalleryImage(gallery, src, index) {
  ...
  const label = gallery.dataset.galleryLabel || "Categorie";

  if (mainImage) {
    mainImage.src = src;
    mainImage.alt = `${label} - imagine galerie ${index + 1}`;
  }
...
function renderProductGallery(gallery, imageSources) {
  ...
  sources.forEach((src, index) => {
    ...
    thumb.setAttribute("aria-label", `${label} - imagine ${index + 1}`);
...
if (categoryWorldPage) {
  ...
  const setCategoryGalleryImage = (gallery, src, index) => {
    ...
    const label = gallery.dataset.categoryLabel || "Categorie";

    if (mainImage && src) {
      mainImage.src = src;
      mainImage.alt = `${label} - imagine ${index + 1}`;
    }
...
if (statNumbers.length) {
  const formatStatNumber = (value) => Math.round(value).toLocaleString("ro-RO");
...
const setupCommunityTeaserSlider = () => {
  ...
  const slides = firstSetImages.map((image, index) => ({
    ...
    alt: image.alt || `Inspirație comunitate Layero ${index + 1}`,
  }));
  ...
  const ensureLightbox = () => {
    ...
    lightbox.setAttribute("aria-label", "Galerie comunitate");
    lightbox.innerHTML = `
      <button class="community-lightbox__button community-lightbox__close" type="button" aria-label="Închidere">
        <svg class="icon" aria-hidden="true"><use href="#icon-x"></use></svg>
      </button>
      <button class="community-lightbox__button community-lightbox__prev" type="button" aria-label="Imaginea anterioară">
      ...
      <button class="community-lightbox__button community-lightbox__next" type="button" aria-label="Imaginea următoare">
        <svg class="icon" aria-hidden="true"><use href="#icon-arrow-right"></use></svg>
      </button>
  ...
  allImages.forEach((image, index) => {
    ...
    image.setAttribute("aria-label", `Deschide ${slides[slideIndex]?.alt || "imagine comunitate"}`);
...
const setupHeaderScrollState = () => {
  ...
  const update = () => {
    ...
    // hiszterézis: máshol vált be și ki, így a küszöb körül nem ugrál
